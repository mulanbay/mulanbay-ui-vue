...
export function dateDiff(sDate1, sDate2) {
  ...
  var iDays = Math.round(Math.abs(oDate1 - oDate2) / 1000 / 60 / 60 / 24); //把相差的毫秒数转换为天数
  return iDays;
}
...
export const datePickerOptions = {
  shortcuts: [{
    ...
    onClick(picker) {
      ...
      const end = getLastDayOfWeek(today);
      const start = getFirstDayOfWeek(today);
    ...
    text: '今年',
    onClick(picker) {
      const yd = getYearDateRange(0);
      picker.$emit('pick', [yd[0], yd[1]]);
    }
  }, {
    text: '去年',
    onClick(picker) {
      const yd = getYearDateRange(-1);
      picker.$emit('pick', [yd[0], yd[1]]);
    }
  }, {
    text: '最近十年',
