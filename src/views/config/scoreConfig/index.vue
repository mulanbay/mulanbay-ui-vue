<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      <el-form-item label="组号" prop="name">
        <el-input
      ...
      <el-form-item>
        <el-button type="query" icon="el-icon-search" size="mini" @click="handleQuery" v-hasPermi="['config:scoreConfig:query']">搜索</el-button>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleCreate"
          v-hasPermi="['config:scoreConfig:create']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['config:scoreConfig:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['config:scoreConfig:delete']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['config:scoreConfig:export']"
        >导出</el-button>
      </el-col>
    </el-row>
...
<script>
import {fetchList,getScoreConfig,createScoreConfig,updateScoreConfig,deleteScoreConfig} from "@/api/config/scoreConfig";
...
export default {
  ...
  created() {
    this.handleReceiveData(this.scoreConfigGroupData);
  ...
  //监听父层带过来的账户信息值
  watch:{
    scoreConfigGroupData(newVal,oldVal){
      this.scoreConfigGroupData = newVal;
      this.handleReceiveData(this.scoreConfigGroupData);
    }
  },
  methods: {
    ...
    handleReceiveData(data){
      this.queryParams.groupId=this.scoreConfigGroupData.groupId;
      this.getList();
    },
    /** 查询列表 */
    getList() {
      ...
      fetchList(this.queryParams).then(
        response => {
          this.scoreConfigList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateScoreConfig(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            createScoreConfig(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有购买来源数据项?', "警告", {
          ...
          return exportScoreConfig(queryParams);
        }).then(response => {
