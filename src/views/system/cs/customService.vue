<template>
  <div class="app-container" ref="main" >
    <div class="lite-chatbox" id="res" style="height: 450px;background-color: WhiteSmoke;">
      <div v-for="(item,i) in dataList">
        ...
        <div v-else-if="item.type=='csmg'" class="cleft cmsg">
          <img class="headIcon radius" ondragstart="return false;"  oncontextmenu="return false;"  src="../../../assets/image/cs.png"/>
          <span class="name">{{item.name}}</span>
          <span class="content">{{item.content}}</span>
        </div>
    ...
    <el-input v-model="reqText" style="width: 480px" @keyup.enter.native="sendReq" placeholder="请输入" />
    <el-button type="primary" size="mini" @click="sendReq">发 送</el-button>
    <el-button type="primary" size="mini" @click="clearCs">清 空</el-button>
  </div>
</template>
