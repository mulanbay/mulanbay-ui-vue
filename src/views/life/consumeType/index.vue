<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        <el-button type="query" icon="el-icon-search" size="mini" @click="handleQuery" v-hasPermi="['life:consumeType:query']">搜索</el-button>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleCreate"
          v-hasPermi="['life:consumeType:create']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['life:consumeType:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['life:consumeType:delete']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['life:consumeType:export']"
        >导出</el-button>
      </el-col>
    </el-row>

    <!--列表数据-->
    <el-table v-loading="loading" :data="consumeTypeList" @selection-change="handleSelectionChange">
...
<script>
import {fetchList,getConsumeType,createConsumeType,updateConsumeType,deleteConsumeType} from "@/api/life/consumeType";
  ...
  methods: {
    ...
    getList() {
      ...
      fetchList(this.queryParams).then(
        response => {
          this.consumeTypeList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getConsumeType(id).then(response => {
        this.form = response;
        this.open = true;
        this.title = "修改";
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateConsumeType(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            createConsumeType(this.form).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有购买来源数据项?', "警告", {
          ...
          return exportConsumeType(queryParams);
        }).then(response => {
