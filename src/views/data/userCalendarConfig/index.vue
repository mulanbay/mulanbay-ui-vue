<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        <el-button type="query" icon="el-icon-search" size="mini" @click="handleQuery" v-hasPermi="['data:userCalendarConfig:query']">搜索</el-button>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleCreate"
          v-hasPermi="['data:userCalendarConfig:create']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['data:userCalendarConfig:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['data:userCalendarConfig:delete']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['data:userCalendarConfig:export']"
        >导出</el-button>
      </el-col>
    </el-row>

    <!--列表数据-->
    <el-table v-loading="loading" :data="userCalendarConfigList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="750px" append-to-body customClass="customDialogCss">
      <el-form ref="form" :model="form" :rules="rules" label-width="90px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="选取模板" prop="tmpUserCalendarConfigId">
             <treeselect
              v-model="form.tmpUserCalendarConfigId"
              @input="loadTemplate"
              style="width: 580px"
    ...
    <el-dialog :title="statValueConfigListTitle" width="850px" :visible.sync="statValueConfigListVisible"  append-to-body>
      <stat-value-list
        :parentData="userCalendarConfigData"
      />
    </el-dialog>
...
<script>
  import {fetchList,getUserCalendarConfig,getUserCalendarConfigTree,createUserCalendarConfig,updateUserCalendarConfig,deleteUserCalendarConfig} from "@/api/data/userCalendarConfig";
  import Treeselect from "@riophae/vue-treeselect";
  import "@riophae/vue-treeselect/dist/vue-treeselect.css";
  import StatValueList from '../../report/statValueConfig/index'

export default {
  name: "UserCalendarConfig",
  components: {
    Treeselect,
  ...
  methods: {
    loadTemplate(newVal){
      ...
      getUserCalendarConfig(newVal).then(response => {
        response.id=undefined;
        this.form = response;
        //不重新设置会导致树形选择框不显示
        this.form.tmpUserCalendarConfigId=newVal;
      });
    },
    /** 查询搜索条件中的模板下拉树结构 */
    getUserCalendarConfigTreeselect() {
    ...
    handleStatValueList(row){
      ...
      this.userCalendarConfigData = Object.assign({}, this.userCalendarConfigData, {
        fid: row.id,
        type:'CALENDAR'
      });
    },
    ...
    getList() {
      ...
      fetchList(this.queryParams).then(
        response => {
          this.userCalendarConfigList = response.rows;
    ...
    handleCreate() {
      this.reset();
      this.getUserCalendarConfigTreeselect();
    ...
    handleUpdate(row) {
      ...
      getUserCalendarConfig(id).then(response => {
        this.form = response;
        this.open = true;
        this.title = "修改";
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateUserCalendarConfig(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            createUserCalendarConfig(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除编号为"' + ids + '"的数据项?', "警告", {
          ...
          return deleteUserCalendarConfig(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有购买来源数据项?', "警告", {
          ...
          return exportUserCalendarConfig(queryParams);
        }).then(response => {
